...
# Un rebase qui échoue est annulé (git rebase --abort), une notification part via daily-notify.js --test, et le script sort en erreur.
...
# Verrou partagé avec render-pending / daily-drafts / weekly-stats (flock dans les unités systemd)
exec 9>"$LOCK"
...
notify() {
  ...
  echo "$now" > "$stamp"
  (cd "$INFRA" && node src/publish/daily-notify.js --test "git-sync VPS : $1" >/dev/null 2>&1) || say "notification impossible (NOTIFY_CHANNEL ?)"
}
...
if ! git fetch -q "$REMOTE" "$BRANCH"; then say "fetch impossible (réseau ? clé de déploiement ?)"; notify "fetch impossible ($REMOTE/$BRANCH)"; exit 1; fi
if ! git pull -q --rebase --autostash "$REMOTE" "$BRANCH"; then
  say "CONFLIT au rebase → abandon"
  git rebase --abort 2>/dev/null || true   # l'autostash est réappliqué par l'abort
  notify "conflit git au rebase sur $BRANCH : à résoudre à la main (ssh, cd $APP_DIR, git status)"
  exit 1
fi
...
if git push -q "$REMOTE" "HEAD:$BRANCH"; then say "push ok"; exit 0; fi
say "push refusé, nouvel essai après rebase"
if git pull -q --rebase --autostash "$REMOTE" "$BRANCH" && git push -q "$REMOTE" "HEAD:$BRANCH"; then say "push ok (2e essai)"; exit 0; fi
git rebase --abort 2>/dev/null || true
notify "push impossible sur $BRANCH après 2 essais (clé en lecture seule ? branche protégée ?)"
exit 1
