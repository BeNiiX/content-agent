...
secrets, ne modifie pas `infra/`**. Aucun connecteur n'est nécessaire (retirer Higgsfield et les autres du formulaire :
...
08:00 (`content-weekly-stats`) et pousse le tout dans les 10-30 min ; la routine veille-hebdo annote le digest à 09:00, la revue
...
| Champ | Valeur |
|---|---|
| **Nom** | `content · veille-hebdo` (ou `production-specs · <compte>`, `revue-hebdo`) — préfixer du slug de `infra/config/project.json` si plusieurs apps |
...
| **Connecteurs** | **tout retirer** (Higgsfield, Gmail, Supabase…) : ces routines n'en ont pas besoin, et un connecteur inclus est utilisable sans confirmation pendant le run |
