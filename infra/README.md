...
Node ≥ 20, **aucune dépendance npm**. Outils système : `yt-dlp`, `ffmpeg`, `whisper-cpp` (Homebrew) + modèle `models/ggml-small.bin` (huggingface ggerganov/whisper.cpp). OCR : `swiftc -O -o bin/ocr scripts/ocr.swift` (framework Vision, macOS ; absent sous Linux → `extract.js` dégrade proprement, sans texte à l'écran). Rendu d'images : `python3 -m venv .venv && .venv/bin/pip install pillow`. Voix des vidéos F04 : `.venv/bin/pip install edge-tts` (voix neurales Microsoft, en ligne, sans clé — Denise par défaut) ; option hors ligne `piper-tts` + modèles onnx dans `models/piper/` (non versionnés, huggingface rhasspy/piper-voices). Le runtime de production est un VPS Linux (`deploy/README.md`) ; le Mac reste le poste de création.
...
| `config/accounts.json` | Comptes TikTok : handle, appareil, identifiant chez le fournisseur d'envoi (`<backend>_account_id`), DA, description fixe des carrousels, flags pause |
...
bash scripts/daily-stats.sh                                    # les deux, journal dans data/accounts/weekly.log
...
```
Planifié **sur le VPS** par systemd (`deploy/README.md`) : `content-weekly-veille` (jour de `weekly_stats`, une heure avant) puis `content-daily-stats` (`weekly_stats` de `config/project.json`, lundi 08:00 par défaut) ; `content-git-sync` pousse ensuite `02_VEILLE/`, `STATS.md` et le digest. Vérifier : `systemctl list-timers 'content-*'`, `journalctl -u content-daily-stats -n 50` ; lancer hors horaire : `sudo systemctl start content-daily-stats`. Les exports TikTok bruts arrivent du Mac par `deploy/sync-media.sh --data`. Sur un Mac sans VPS, `node scripts/launchd/make-plists.js` génère des plists launchd équivalents (hors git).
...
node src/calendar/queue-order.js                  # calendrier : ordre d'envoi par compte, concepts qui se suivent ; --interleave alterne les formats, --move EXP-023 --to top|up|down|<n>, --dry-run
                                                 # 2. daily-send.js : envoie chaque item `planned` en brouillon via PUBLISH_BACKEND (Node pur), attend la confirmation de la plateforme, daily-result.js trace + mark-draft
...
node src/publish/daily-notify.js --alert "texte"  # vraie alerte d'un job du VPS : titre d'alerte, priorité haute (git-sync, auth-check)
```
Planifié **sur le VPS** par `content-daily-drafts.timer` (tous les jours à `daily_hour`, `deploy/README.md` ; `journalctl -u content-daily-drafts`, `sudo systemctl start content-daily-drafts` pour lancer tout de suite). Sur un Mac qui planifie en local (`scripts/launchd/make-plists.js`, plists hors git) : Mac allumé et session ouverte requis, et si le projet est dans un dossier protégé (Bureau, Documents) il faut donner **Accès complet au disque** à `/bin/bash` (Réglages Système → Confidentialité et sécurité), sinon launchd échoue avec « Operation not permitted » (exit 126). Notification : **ntfy** (app téléphone abonnée au sujet secret `NTFY_TOPIC`, une notification par compte, expéditeur = ntfy donc alerte réelle ; titre par défaut = `notify_title` de `project.json`) ; iMessage inutile (message envoyé à soi-même, aucune alerte) et impossible sous Linux. Aucun modèle dans la boucle : tout est déterministe (Node pur), le fournisseur d'envoi est choisi par `PUBLISH_BACKEND` dans `.env`. La soumission prend quelques secondes mais la plateforme met 5 à 20 min à livrer le brouillon : `daily-send.js` attend la confirmation réelle jusqu'à `DAILY_SEND_WAIT_MIN` avant de notifier (d'où un timer à 17:45 pour des brouillons vers 18:00). Idempotent : une relance le même jour ne renvoie pas sur un compte déjà servi ; un compte sans stock est signalé dans le message, jamais servi deux fois. Un échec TikTok est signalé et le contenu reste `prêt` pour le lendemain. Journal : `data/publish/daily/<date>.log`.
...
node src/publish/daily-plan.js [--date AAAA-MM-JJ] [--accounts <slug>,…]  # plan du soir : 1 item par compte (premier contenu `prêt` de sa file)
node src/publish/daily-send.js [--exp EXP-015] [--accounts] [--connect-url <slug>] [--probe]   # envoi des brouillons via PUBLISH_BACKEND ; --accounts liste les comptes du fournisseur, --connect-url donne le lien d'autorisation TikTok
...
```
TikTok = brouillons en boîte de réception par l'API créateur (Login Kit), via le fournisseur choisi dans `PUBLISH_BACKEND` — jamais l'API Business, jamais de publication directe. Pipeline complet et connexion d'un compte : `src/publish/README.md`, `src/publish/CONNEXION_TIKTOK.md`.
...
.venv/bin/python scripts/partner-quiz-video.py ../08_ACCOUNTS/<account>/specs/F04/F04-<angle>-<nn>.json [--preview planche.jpg]   # FORMAT-04 test du partenaire (proposé) : photo + hook incrusté et dit par la voix → N × (carte lue par la voix + chrono 3-2-1-0 avec tic-tac) → CTA milieu → fin ; piste audio incluse (macOS `say`, Mac seulement ; --no-tts ailleurs) ; temps réels dans <spec>.plan.json (lu par posts-md.py)
```
